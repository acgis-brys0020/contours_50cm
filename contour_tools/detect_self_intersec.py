...
def detect_self_intersect(geom, min_sep = 5):
    ...
    for i, coord in enumerate(vertices):
        ...
        if first_idx is not None:
            if i - first_idx >= min_sep:
                overlapping.append((coord, first_idx, i)) 
        else:
            seen[coord] = i
...
def detect_knots(out_fc, suspect_artifact_ids, uid, dissolved_fc, out_fc_oid_field):
    ...
    #Returns an empty set and exits the function if no suspect features were found.
    ...
    t1 = time.time() - t0
    arcpy.AddMessage(f"Dissolved features into {arcpy.management.GetCount(dissolved_fc)} features ({t1})")
    ...
    t3 = time.time() - t2
    arcpy.AddMessage(f"Spatial join complete ({t3})")
...
def scan_features(fc, results, loop_threshold):
    ...
    with arcpy.da.SearchCursor(fc, ["OID@", "SHAPE@", "SHAPE@LENGTH"]) as cursor:
        for oid, geom, length in cursor:
            ...
            weird_geometry = detect_self_intersect(geom)
            if weird_geometry:
